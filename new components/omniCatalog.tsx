...
interface PartType {
id: string;
name: string; // e.g., “Screen”, “Battery”, “Charging Port”
}
...
stock: number;
brand: ‘Apple’ | ‘Samsung’; // Explicit brand field
searchIndex: string; // space-delimited lowercase: “iphone 15 pro screen oem battery”
}
...
async function loadParts() {
  try {
    ...
    const validParts = data.parts.filter((part) => {
...
// Step A: Filter by active brand
if (part.brand !== activeBrand) return false;

```
  // Step B: If search is empty, show all brand parts
  if (!searchQuery.trim()) return true;

  // Step C: Omni-Search AND Logic
  // User types "15 pro battery" → split into ["15", "pro", "battery"]
  // Part matches if ALL terms appear in searchIndex
...
(part: PartMaster) => {
// TODO: Integrate with cart context/store (Zustand, Context, etc.)
// This is a stub for the workflow:
// 1. Validate MOQ (minimum 5 units)
...
className={`px-8 py-2 text-sm uppercase tracking-widest font-bold transition-colors rounded-sm ${ activeBrand === brand ? 'bg-[#00E5C0] text-black' : 'text-gray-500 hover:text-gray-300' }`}
>
{brand}
</button>
))}
