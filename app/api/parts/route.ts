...
function centsFromDecimal(value: Prisma.Decimal | null) {
  if (!value) {
    return 0;
  }
...
export async function GET(request: NextRequest) {
  ...
  const whereClauses: Prisma.PartMasterWhereInput[] = [];
  ...
  const where: Prisma.PartMasterWhereInput = whereClauses.length > 0 ? { AND: whereClauses } : {};
